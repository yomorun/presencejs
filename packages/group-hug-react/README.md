...
If the `avatar` image is not specified, [avvvatars](https://avvvatars.com/) will be utilized as the placeholder.
...
| placeholder           | 'shape', 'charactor' | The placeholder image when `avatar` absence                                                               |
...
1. Start local `prscd` dev server: `pnpm run prscd:dev`
2. Start Grouphug code generator tool: `pnpm --filter @yomo/group-hug-react run start:vite`
